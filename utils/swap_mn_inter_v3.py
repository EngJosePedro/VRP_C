...
def _extract_routes_from_packed(sol: torch.Tensor):
    ...
    cnt = torch.zeros((BP,), device=device, dtype=torch.long)
    route_pos = torch.empty((S,), device=device, dtype=torch.long)
    for s in range(S):
        r = int(row_bp[s].item())
        route_pos[s] = cnt[r]
        cnt[r] += 1
...
@torch.no_grad()
def trim_extra_zeros_1d(seq: torch.Tensor, keep_final_zero: bool = True) -> torch.Tensor:
    """
    Remove excesso de zeros no fim de seq (T).

    Se keep_final_zero=True, mantém um zero após o último nó não-zero.
    """
    assert seq.dim() == 1, "seq deve ter shape (T,)"

    nonzero = seq != 0

    if nonzero.any():
        last_nonzero = nonzero.nonzero(as_tuple=True)[0].max().item()
        end = last_nonzero + 2 if keep_final_zero else last_nonzero + 1
        end = min(end, seq.shape[0])
        return seq[:end]
...
def remove_consecutive_zeros_vec(seq: torch.Tensor) -> torch.Tensor:
    """
    Remove zeros consecutivos em seq (T), mantendo apenas o primeiro zero de cada sequência.
    Mantém shape (T) com padding 0 no final.

    seq: Long/Int tensor (T,)
    return: tensor (T,)
    """
    assert seq.dim() == 1, "seq deve ter shape (T,)"
    ...
    dup = (curr == 0) & (prev == 0)  # (T-1,)

    keep = torch.ones_like(seq, dtype=torch.bool)  # (T,)
    keep[1:] = ~dup

    # 2) posições compactadas
    pos = keep.cumsum(dim=0) - 1  # (T,)

    # 3) scatter compactado
    out = torch.zeros((T,), device=device, dtype=seq.dtype)

    idx = torch.where(keep)[0]
    out[pos[idx]] = seq[idx]

    return trim_extra_zeros_1d(out)
...
def _repack_rows_from_route_list(
    ...
    """
    Functional first version: repacks only rows that changed.
    There is still a Python loop over changed rows.
    """
    seq_out = seq.clone()
    BP, T = seq.shape
    
    # Build route lists per row
    changed_set = set(int(r.item()) for r in selected_rows)
    
    row_to_routes = {}
    for r in changed_set:
        idx = torch.nonzero(row_bp == r, as_tuple=False).squeeze(-1)
        #print(idx)
        order = route_pos[idx].argsort(stable=True)
        #print(order)
        row_to_routes[r] = idx[order]
    

    row_to_sel = {}
    for k in range(selected_rows.numel()):
        r = int(selected_rows[k].item())
        row_to_sel[r] = k
    
    for r in changed_set:
        k = row_to_sel[r]
        ga = int(selected_route_a[k].item())
        gb = int(selected_route_b[k].item())
        
        route_ids = row_to_routes[r]
        
        blocks = []
        for gid in route_ids.tolist():
            if gid == ga:
                L = int(selected_new_la[k].item())
                enter_block = selected_new_ra[k, :L]
                if enter_block.shape[0]>2:
                    blocks.append(enter_block)
            elif gid == gb:
                L = int(selected_new_lb[k].item())
                enter_block = selected_new_rb[k, :L]
                if enter_block.shape[0]>2:
                    blocks.append(enter_block)
            else:
                L = int(lengths[gid].item())
                enter_block = routes[gid, :L]
                if enter_block.shape[0]>2:
                    blocks.append(enter_block)
                
        
        packed = torch.cat(blocks, dim=0)
        packed = remove_consecutive_zeros_vec(packed)
       
        if packed.numel() > T:
            raise RuntimeError("Reconstrução da solução excedeu T. Verifique o packing de entrada.")

        seq_out[r].zero_()
        seq_out[r, :packed.numel()] = packed
...
def apply_swap_mn_2opt_inter_routes_pytorch(
    ...
    *,
    ...
    for it in range(max_iters):
        ...
        # pair generation (ordered pairs) within each packed solution row
        same_row = row_bp[:, None] == row_bp[None, :]
        diff_route = torch.arange(S, device=device)[:, None] != torch.arange(S, device=device)[None, :]
        pair_mask = same_row & diff_route
        pair_a, pair_b = torch.nonzero(pair_mask, as_tuple=True)
        if pair_a.numel() == 0:
            break

        # stable order close to original nested loops:
        # a ascending, b ascending because nonzero(row-major) already behaves like that.
