...
@torch.no_grad()
def _batched_route_costs_by_problem_index(
    dist: torch.Tensor,
    ...
    """
    Calcula custo de rotas sem materializar distS = dist[problem_idx] com shape (S,N,N).
    Memória temporária O(S*L), não O(S*N*N).
    """
    S, Lmax = routes.shape
    ...
    N = dist.size(-1)
    pos = torch.arange(Lmax - 1, device=device)[None, :].expand(S, Lmax - 1)
    mask = pos < (lengths[:, None] - 1)

    u = routes[:, :-1].clamp_min(0)
    v = routes[:, 1:].clamp_min(0)
    lin = u * N + v

    dist_flat = dist.reshape(dist.size(0), N * N)
    edge = dist_flat[problem_idx[:, None], lin]
    return (edge * mask.to(dtype)).sum(dim=1)


@torch.no_grad()
def _select_first_improvement_per_row(
    BP: int,
    owner_c: torch.Tensor,
    delta: torch.Tensor,
    improve: torch.Tensor,
    row_has_first: torch.Tensor,
):
    device = delta.device
    ...
    return rows_sel, idx_sel


@torch.no_grad()
def _select_best_improvement_per_row_in_chunk(
    BP: int,
    owner_c: torch.Tensor,
    delta: torch.Tensor,
    improve: torch.Tensor,
    best_delta_row: torch.Tensor,
):
    device = delta.device
    dtype = delta.dtype
    cand_idx = torch.arange(delta.numel(), device=device, dtype=torch.long)
    ...
    return rows_sel[good], idx_sel[good]
...
def apply_swap_mn_2opt_inter_routes_pytorch(
    ...
    *,
    ...
    """
    Versão com menor consumo de memória de swap(m,n) inter-rotas + 2-opt.

    Mudanças principais:
      1) route_cost sem dist[row_bp // P] para todas as rotas: O(S*L), não O(S*N*N).
      2) candidatos factíveis avaliados em chunks controlados por candidate_chunk_size.
      3) stacked_dist limitado a (2*candidate_chunk_size, N, N).

    Para B=128, P=2, N=201:
      candidate_chunk_size=256 -> stacked_dist ~ 79 MB.
      candidate_chunk_size=128 -> stacked_dist ~ 39 MB.
    """
    assert sol.dim() == 3, "sol deve ter shape (B,P,T)"
    assert dist.dim() == 3, "dist deve ter shape (B,N,N)"
    assert candidate_chunk_size > 0
    ...
    for it in range(max_iters):
        ...
        route_demand = demand_bp[row_bp]
        route_load = _route_loads(routes, lengths, route_demand)
        route_problem_idx = row_bp // P
        route_cost = _batched_route_costs_by_problem_index(dist, route_problem_idx, routes, lengths)
        ...
        for m in range(0, max_m + 1):
            for n in range(0, max_n + 1):
                ...
                if first_improve:
                    keep_unresolved = ~row_has_first[owner]
                    if not keep_unresolved.any():
                        continue
                    pa = pa[keep_unresolved]
                    pb = pb[keep_unresolved]
                    owner = owner[keep_unresolved]
                    la = la[keep_unresolved]
                    lb = lb[keep_unresolved]
                ...
                valid_sa = sa_grid[None, :] <= sa_max[:, None]
                valid_sb = sb_grid[None, :] <= sb_max[:, None]
                cand_mask = valid_sa[:, :, None] & valid_sb[:, None, :]
                if not cand_mask.any():
                    continue

                pair_idx_all, sa_idx_all, sb_idx_all = torch.nonzero(cand_mask, as_tuple=True)
                Ctot = pair_idx_all.numel()
                if Ctot == 0:
                    continue

                for c0 in range(0, Ctot, candidate_chunk_size):
                    #print(it, m, n, c0, Ctot)
                    c1 = min(Ctot, c0 + candidate_chunk_size)
                    pair_idx = pair_idx_all[c0:c1]
                    sa_idx = sa_idx_all[c0:c1]
                    sb_idx = sb_idx_all[c0:c1]
                    ...
                    if first_improve:
                        keep_unresolved_c = ~row_has_first[owner_c]
                        if not keep_unresolved_c.any():
                            continue
                        pa_c = pa_c[keep_unresolved_c]
                        pb_c = pb_c[keep_unresolved_c]
                        owner_c = owner_c[keep_unresolved_c]
                        sa_idx = sa_idx[keep_unresolved_c]
                        sb_idx = sb_idx[keep_unresolved_c]
                    ...
                    pa_c = pa_c[feasible]
                    pb_c = pb_c[feasible]
                    owner_c = owner_c[feasible]
                    new_ra = new_ra[feasible]
                    new_rb = new_rb[feasible]
                    new_la = new_la[feasible]
                    new_lb = new_lb[feasible]
                    ...
                    new_ra = _pad_routes_to_len(new_ra, Lcand)
                    new_rb = _pad_routes_to_len(new_rb, Lcand)
                    stacked_routes = torch.cat([new_ra, new_rb], dim=0)
                    stacked_lengths = torch.cat([new_la, new_lb], dim=0)

                    prob_a = row_bp[pa_c] // P
                    prob_b = row_bp[pb_c] // P
                    stacked_problem_idx = torch.cat([prob_a, prob_b], dim=0)
                    stacked_dist = dist[stacked_problem_idx]
                    ...
                    stacked_opt, _ = _apply_iterated_two_opt_on_routes(
                        ...
                        i_chunk=i_chunk,
                        k_chunk=k_chunk,
                        s_chunk=min(s_chunk, max(1, stacked_routes.size(0))),
                    )
                    ...
                    new_cost = (
                        _batched_route_costs_by_problem_index(dist, prob_a, opt_ra, new_la)
                        + _batched_route_costs_by_problem_index(dist, prob_b, opt_rb, new_lb)
                    )
                    ...
                    if first_improve:
                        rows_sel, idx_sel = _select_first_improvement_per_row(
                            BP, owner_c, delta, improve, row_has_first
                        )
                        if rows_sel.numel() == 0:
                            continue
                        row_has_first[rows_sel] = True
                    else:
                        rows_sel, idx_sel = _select_best_improvement_per_row_in_chunk(
                            BP, owner_c, delta, improve, best_delta_row
                        )
                        if rows_sel.numel() == 0:
                            continue
                    ...
                    best_new_ra, best_new_rb = _write_selected_best_routes(
                        best_new_ra,
                        best_new_rb,
                        best_new_la,
                        best_new_lb,
        ...
        if debug:
            mem = ""
            if torch.cuda.is_available() and seq.is_cuda:
                mem = f" mem_alloc={torch.cuda.memory_allocated() / 1024**3:.3f}GB mem_reserved={torch.cuda.memory_reserved() / 1024**3:.3f}GB"
            print(
                f"iter={it} improved_rows={int(improve_rows.sum().item())} "
                f"total_delta={best_delta_row[improve_rows].sum().item():.6f}{mem}"
            )
