...
def get_lkh_executable_path(path):
    executable = path#r"C:\Users\jose-\source\repos\LKH3\x64\Release\LKH3.exe"
    assert os.path.isfile(executable), f"LKH não encontrado: {executable}"
    return executable


def get_lkh_executable(url="http://www.akira.ruc.dk/~keld/research/LKH-3/LKH-3.0.4.tgz", executable_path = None):

    if executable_path is not None:
        return get_lkh_executable_path(executable_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--executable_path", default = None, help="")
    ...
    for dataset_path in opts.datasets:
        ...
        if method == "lkh":
            executable = get_lkh_executable(executable_path = opts.executable_path)
