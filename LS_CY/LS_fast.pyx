...
def _check_routes(
    ...
    cdef int n = n1 - 1
    cdef object used = np.zeros(n1, dtype=np.uint8)
    ...
    for r in routes:
        if len(r) < 3 or r[0] != 0 or r[len(r) - 1] != 0:
            continue
...
def two_opt_best_route(route, cnp.ndarray[DTYPE_t, ndim=2] dist, bint return_cost=False):
    ...
    if n < 4:
        if return_cost:
            return route, 0.0, _route_cost_c(route, dist)
        return route, 0.0
...
def apply_swap_mn_with_2opt(
    ...
    *,
    ...
    if not (new_ra and new_ra[0] == 0 and new_ra[len(new_ra) - 1] == 0):
        return None
    if not (new_rb and new_rb[0] == 0 and new_rb[len(new_rb) - 1] == 0):
        return None
